...
export class PdfService {
      ...
      switch (el.type) {
        case 'text': {
          ...
          const node: any = {
            ...
            // Fix vertical text alignment offset
            margin: [0, 0, 0, 0],
  ...
  generateCode(template: Template, records: DataRecord[], isBulk: boolean): string {
    ...
    // Prepare background image variables for optimized output
    const bgVars = this.prepareBackgroundVars(template);
...
`;
  ...
  private generateLoopCode(template: Template): string {
    ...
    if (template.pages && template.hasBackPage && template.pages.length >= 2) {
      ...
      if (frontPage.settings.backgroundImage) {
        ...
        lines.push(`    image: frontBackgroundImageData,`);
      ...
      if (backPage.settings.backgroundImage) {
        lines.push(`  content.push({`);
        lines.push(`    image: backBackgroundImageData,`);
      ...
      if (currentPage.settings.backgroundImage) {
        ...
        lines.push(`  content.push({`);
        lines.push(`    image: backgroundImageData,`);
  ...
  private addElementCode(lines: string[], el: any): void {
    switch (el.type) {
      case 'text': {
        ...
        lines.push(`    lineHeight: ${el.style.lineHeight},`);
        lines.push(`    opacity: ${el.style.opacity},`);
        lines.push(`    margin: [0, 0, 0, 0],`);
        lines.push(`    preserveLeadingSpaces: true,`);
        if (el.style.backgroundColor && el.style.backgroundColor !== 'transparent') {
          lines.push(`    background: ${JSON.stringify(el.style.backgroundColor)},`);
        }
        if (el.style.underline || el.style.strike) {
          const decorations: string[] = [];
          if (el.style.underline) decorations.push('underline');
          if (el.style.strike) decorations.push('lineThrough');
          lines.push(`    decoration: ${JSON.stringify(decorations)},`);
        }
        if (el.style.decorationStyle && el.style.decorationStyle !== 'none') {
          lines.push(`    decorationStyle: ${JSON.stringify(el.style.decorationStyle)},`);
        }
        if (el.style.decorationColor) {
          lines.push(`    decorationColor: ${JSON.stringify(el.style.decorationColor)},`);
        }
        if (el.style.characterSpacing && el.style.characterSpacing > 0) {
          lines.push(`    characterSpacing: ${el.style.characterSpacing},`);
        }
        if (el.style.linkUrl) {
          lines.push(`    link: resolveVars(${JSON.stringify(el.style.linkUrl)}, record),`);
        }
        lines.push(`  });`);
        break;
      }
      case 'image': {
        ...
        if (!src) break;
        lines.push(`  content.push({`);
        lines.push(`    image: resolveVars(${JSON.stringify(src)}, record),`);
      ...
      case 'rectangle': {
        ...
        if (hasBackground || hasBorder) {
          ...
          lines.push(`      x: 0, y: 0,`);
          lines.push(`      w: ${Math.round(el.width)}, h: ${Math.round(el.height)},`);
          if (hasBackground) lines.push(`      color: ${JSON.stringify(el.style.backgroundColor)},`);
          lines.push(`      lineWidth: ${hasBorder ? el.style.borderWidth : 0},`);
          if (hasBorder) lines.push(`      lineColor: ${JSON.stringify(el.style.borderColor)},`);
          lines.push(`    }],`);
          lines.push(`    absolutePosition: { x: ${Math.round(el.x)}, y: ${Math.round(el.y)} },`);
          lines.push(`    width: ${Math.round(el.width)},`);
          lines.push(`  });`);
      ...
      case 'line': {
        ...
        lines.push(`      x1: 0, y1: ${el.style.lineWidth / 2 || 1},`);
        lines.push(`      x2: ${Math.round(el.width)}, y2: ${el.style.lineWidth / 2 || 1},`);
        lines.push(`      lineWidth: ${el.style.lineWidth || 1},`);
        lines.push(`      lineColor: ${JSON.stringify(el.style.lineColor || '#000000')},`);
        lines.push(`      lineCap: ${JSON.stringify(el.style.lineCapStyle || 'butt')},`);
        if (el.style.lineDash > 0) {
          lines.push(`      dash: { length: ${el.style.lineDash} },`);
        }
        lines.push(`    }],`);
        lines.push(`    absolutePosition: { x: ${Math.round(el.x)}, y: ${Math.round(el.y)} },`);
        lines.push(`    width: ${Math.round(el.width)},`);
      ...
      case 'ellipse': {
        ...
        lines.push(`  content.push({`);
        lines.push(`    canvas: [{`);
        lines.push(`      type: 'ellipse',`);
        lines.push(`      x: ${el.width / 2},`);
        lines.push(`      y: ${el.height / 2},`);
        lines.push(`      r1: ${el.width / 2},`);
        lines.push(`      r2: ${el.height / 2},`);
        if (hasBackground) lines.push(`      color: ${JSON.stringify(el.style.backgroundColor)},`);
        lines.push(`      lineWidth: ${hasBorder ? el.style.borderWidth : 0},`);
        if (hasBorder) lines.push(`      lineColor: ${JSON.stringify(el.style.borderColor)},`);
        lines.push(`    }],`);
        lines.push(`    absolutePosition: { x: ${Math.round(el.x)}, y: ${Math.round(el.y)} },`);
        lines.push(`    width: ${Math.round(el.width)},`);
        lines.push(`    height: ${Math.round(el.height)},`);
        lines.push(`  });`);
        break;
      }
      case 'table': {
        if (el.tableData) {
          ...
          lines.push(`  content.push({`);
          lines.push(`    table: {`);
          lines.push(`      headerRows: ${tableData.headerRows || 1},`);
          lines.push(`      widths: ${JSON.stringify(tableData.colWidths || ['*'])},`);
          lines.push(`      body: ${JSON.stringify(tableData.cells.map((row: any[], ri: number) =>
            row.map((cell: any) => ({
              text: this.resolveVars(cell.text, {}), // Note: variables not resolved in generated code
              rowSpan: cell.rowSpan,
              colSpan: cell.colSpan,
              fillColor: cell.fillColor || (ri % 2 === 1 ? tableData.alternateRowFill : undefined),
              color: cell.color,
      ...
      case 'qrcode': {
        const qrContent = el.content;
        lines.push(`  content.push({`);
        lines.push(`    qr: resolveVars(${JSON.stringify(qrContent || 'QR')}, record),`);
        lines.push(`    fit: ${el.style.qrFit || el.width},`);
        lines.push(`    foreground: ${JSON.stringify(el.style.qrForeground || '#000000')},`);
        lines.push(`    background: ${JSON.stringify(el.style.qrBackground || '#ffffff')},`);
        lines.push(`    eccLevel: ${JSON.stringify(el.style.qrEcc || 'M')},`);
        lines.push(`    absolutePosition: { x: ${Math.round(el.x)}, y: ${Math.round(el.y)} },`);
        lines.push(`  });`);
        break;
      }
      case 'list': {
        const listItems = (el.listItems || []).map((item: any) => this.resolveVars(item.text, {})); // Note: not resolving vars
        if (el.listType === 'ol') {
          lines.push(`  content.push({`);
          lines.push(`    ol: ${JSON.stringify(listItems)},`);
          lines.push(`    type: ${JSON.stringify(el.listStyle || 'decimal')},`);
          lines.push(`    color: ${JSON.stringify(el.listMarkerColor)},`);
          lines.push(`    absolutePosition: { x: ${Math.round(el.x)}, y: ${Math.round(el.y)} },`);
          lines.push(`    width: ${Math.round(el.width)},`);
          lines.push(`  });`);
        } else {
          lines.push(`  content.push({`);
          lines.push(`    ul: ${JSON.stringify(listItems)},`);
          lines.push(`    type: ${JSON.stringify(el.listStyle || 'disc')},`);
          lines.push(`    color: ${JSON.stringify(el.listMarkerColor)},`);
          lines.push(`    absolutePosition: { x: ${Math.round(el.x)}, y: ${Math.round(el.y)} },`);
          lines.push(`    width: ${Math.round(el.width)},`);
          lines.push(`  });`);
      ...
      case 'columns': {
        const columns = (el.columnDefs || []).map((col: any) => ({
          text: this.resolveVars(col.text, {}), // Note: not resolving
          width: col.width,
          fontSize: col.fontSize,
          color: col.color,
          bold: col.bold,
        ...
        lines.push(`    columnGap: ${el.columnGap || 20},`);
        lines.push(`    absolutePosition: { x: ${Math.round(el.x)}, y: ${Math.round(el.y)} },`);
        lines.push(`  });`);
        break;
      }
      case 'svg': {
        const svgContent = el.content;
        lines.push(`  content.push({`);
        lines.push(`    svg: resolveVars(${JSON.stringify(svgContent)}, record),`);
        lines.push(`    fit: [${Math.round(el.width)}, ${Math.round(el.height)}],`);
        lines.push(`    absolutePosition: { x: ${Math.round(el.x)}, y: ${Math.round(el.y)} },`);
        lines.push(`  });`);
        break;
      }
  ...
  private prepareBackgroundVars(template: Template): string {
    let vars = '';
    if (template.pages && template.hasBackPage && template.pages.length >= 2) {
      const front = template.pages[0].settings.backgroundImage;
      const back = template.pages[1].settings.backgroundImage;
      if (front) {
        vars += `const frontBackgroundImageData = ${JSON.stringify(front.length > 200 ? `[BASE64_IMAGE_DATA ~${Math.round(front.length / 1024)}KB]` : front)};\n`;
      }
      if (back) {
        vars += `const backBackgroundImageData = ${JSON.stringify(back.length > 200 ? `[BASE64_IMAGE_DATA ~${Math.round(back.length / 1024)}KB]` : back)};\n`;
      }
    } else {
      const page = template.pages ? template.pages[0].settings : template.page;
      if (page.backgroundImage) {
        vars += `const backgroundImageData = ${JSON.stringify(page.backgroundImage.length > 200 ? `[BASE64_IMAGE_DATA ~${Math.round(page.backgroundImage.length / 1024)}KB]` : page.backgroundImage)};\n`;
      }
    }
    if (vars) {
      vars = '// Background images (replace [BASE64_IMAGE_DATA...] with your actual base64 data)\n' + vars + '\n';
    }
    return vars;
  }
